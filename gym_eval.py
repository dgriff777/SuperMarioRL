...
try:
    for i_episode in range(args.num_episodes):
        ...
        if args.record_video:
            video_recorder = VideoRecorder(player.env, f"vid_log/{env_id}{i_episode}.mp4", enabled=True)
        ...
        while 1:
            ...
            if player.done:
                if not player.env.unwrapped.is_single_stage_env or not player.env.was_real_done: #player.info["life"]!=255: #
                    if args.load_rms_stats:
                        player.env.load("test_env_data")
                        player.env.set_training_off()
                    player.state = player.env.reset()
                    ...
                    log.info("{}".format(crayons.yellow(f"Time {time.strftime('%Hh %Mm %Ss', time.gmtime(time.time() - start_time))}, episode reward {reward_sum}, episode length {player.eps_len}, reward mean {reward_mean:.4f}", bold=True)))
                    player.eps_len = 0
                    if args.record_video:
